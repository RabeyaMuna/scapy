...
from scapy.compat import Self, bytes_encode, orb, raw
from scapy.config import _version_checker, conf
...
class Packet(BasePacket, _CanvasDumpExtended, metaclass=Packet_metaclass):
        ...
        for field in self.fields_desc:
            ...
            if isinstance(value, RawVal):
                ...
                try:
                    ...
                    raise ValueError(
                        "Invalid value for field %r: %r (%s)" % (fname, value, e)
                    ) from e
    ...
    def do_dissect_payload(self, s):
        ...
        if s:
            ...
            try:
                ...
                # Do not re-raise arbitrary exceptions from dissectors. Log them
                # when debug is enabled and fall back to a raw layer to avoid
                # aborting higher-level processing on parsing errors.
                if conf.debug_dissector:
                    if issubtype(cls, Packet):
                        log_runtime.error("%s dissector failed: %s", cls.__name__, _e)
                    else:
                        log_runtime.error(
                ...
                # Only propagate truly fatal exceptions (KeyboardInterrupt handled above).
                p = conf.raw_layer(s, _internal=1, _underlayer=self)
            self.add_payload(p)
    ...
    def _show_or_dump(
        ...
        if dump:
            from scapy.themes import AnsiColorTheme, ColorTheme
